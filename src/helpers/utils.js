...
export function getDateWithTheLastHour() {
...
/**
 * Dar formato de Código con ceross
 * @param {number} value
 * @param {number} totalDigits
 * @param {number} preValue
 * @returns {number}
 */
export function formatCodeWithZeros(value, totalDigits = 5, preValue = 2022) {
  const n = '0'
  const diff = totalDigits - value.toString().length
  return Number(`${preValue}${n.repeat(diff)}${value}`)
}
